...
def get_xls2(row2):
    ...
    templist3 = [
                 ...
                 (4,1,0, 'text', row2[3]),
                 (5,1,0, 'text', str(row2[4])),
                 (6,1,0, 'text', str(row2[5])),
                 (7,1,0, 'text', row2[6]),
                 (8,1,0, 'text', str(row2[7])),
...
def get_xls4(row4):
    ...
    templist7 = [
                 ...
                 (2, 1, 0, 'text', str(row4[1])),
                 (3,1,0, 'text', row4[2]),
                 (4,1,0, 'text', row4[3]),
                 (5,1,0, 'text', str(row4[4])),
                 (6,1,0, 'text', str(row4[5])),
                 (7,1,0, 'text', str(row4[6])),
                 ]
    return templist7
    ...
    def generate_xls_report(self, _p, _xs, data, objects, wb): #wb = workbook
            ...
            if data['filter'] is False:
                if data['filter_user'] is False:
                    ...
                    if len(data['product']) == 0 and data['stage'] is False and data['date_from'] and data['date_to']:
                         
                        self.cr.execute("SELECT ' ',fe_emision,(case when ti_documento='DNI' then '1' else '6' end),nu_documento,no_canilla,\
                        '0', (case when (pbase-exonerado) < 0 then '0' else (pbase-exonerado) end),exonerado,igv,total,(case grupo_producto when '1' then 'EDI' else 'OPT' end) \
                        FROM comprobante WHERE fe_emision >= %s AND fe_emision <= %s",(data['date_from'],data['date_to'],)) 
            ...
            if data['filter'] is False:
                if data['filter_user'] is False:
                    ...
                    if len(data['product']) == 0 and data['stage'] is False and data['date_from'] and data['date_to']:
                        ...
                        self.cr.execute("SELECT co.fe_emision, (case coli.tipo_producto when 'EJEMPLARES' then 'EDI' else 'OPT' end),\
                        coli.co_producto, coli.de_producto, round(sum(coli.q_vendida_neto),3), round(sum(coli.pbase),3), '0', \
                        round(sum(coli.igv) ,3), round(sum(coli.total),3), round(sum(coli.total),3) / sum(coli.q_vendida_neto) \
                        FROM comprobante co, comprobante_linea coli WHERE co.id = coli.comprobante_id AND coli.tipo_producto = 'EJEMPLARES'\
                        AND co.fe_emision >= %s AND co.fe_emision <= %s GROUP BY co.fe_emision,coli.tipo_producto,coli.co_producto,\
                        coli.de_producto",(data['date_from'],data['date_to'],)) 
            ...
            if data['filter'] is False:
                if data['filter_user'] is False:
                    ...
                    if len(data['product']) == 0 and data['stage'] is False and data['date_from'] and data['date_to']:
                        ...
                        self.cr.execute("SELECT co.fe_emision, (case co.grupo_producto when '1' then 'EDI' else 'OPT' end), \
                        coli.co_producto, coli.de_producto, sum(coli.q_vendida_neto), (case coli.tipo_afecto when 'Afecto' then round((sum(coli.q_vendida_neto) /1.18),3) else 0 end), \
                        (case coli.tipo_afecto when 'Afecto' then 0 else round(sum(coli.q_vendida_neto),3) end), \
                        (case coli.tipo_afecto when 'Afecto' then round((sum(coli.q_vendida_neto) /1.18) * .18,3) else 0 end), \
                        (case coli.tipo_afecto when 'Afecto' then round(((sum(coli.q_vendida_neto) / 1.18) * 1.18),3) else round(sum(coli.q_vendida_neto),3) end),\
                        (case coli.tipo_afecto when 'Afecto' then round(((sum(coli.q_vendida_neto) / 1.18) * 1.18),3) / sum(coli.q_vendida_neto) else round(sum(coli.q_vendida_neto)/sum(coli.q_vendida_neto),3) end) \
                        FROM comprobante co, comprobante_linea coli WHERE co.id = coli.comprobante_id\
                        AND coli.tipo_producto <> 'EJEMPLARES' AND \
                        co.fe_emision >= %s AND co.fe_emision <= %s \
                        GROUP BY co.fe_emision, \
                        co.grupo_producto, co.fe_emision, coli.co_producto,coli.de_producto,coli.tipo_afecto \
            ...
            templist4 = [(1,1,0, 'text', 'Fecha Corte Venta'), 
                        (2,1,0, 'text', 'Fecha Corte Compra'),
                        (3,1,0, 'text', 'Codigo Producto'), 
                        (4,1,0, 'text', 'Nombre Producto'),
                        (5,1,0, 'text', 'Suma Compra '),
                        (6,1,0, 'text', 'Suma Venta'),
                        (7,1,0, 'text', 'Diferencia'),]
            row_pos4 += 1
            ...
            if data['filter'] is False:
                if data['filter_user'] is False:
                    ...
                    if len(data['product']) == 0 and data['stage'] is False and data['date_from'] and data['date_to']:
                        ...
                        self.cr.execute("""
                            SELECT
                            fe_corte_venta, max(fe_corte_compra) as fe_corte_compra, codigo_producto, nombre_producto, sum(suma_comprada_neto) as suma_comprada_neto, suma_vendida_neto,
                            sum(suma_comprada_neto) - suma_vendida_neto as diferencia
                            FROM (
                            SELECT 
                            sub_venta.fe_emision as fe_corte_venta, sub_compra.date_planned as fe_corte_compra, 
                            substring(sub_compra.name,2,18) as codigo_producto, sub_venta.de_producto nombre_producto, 
                            suma_comprada_neto, suma_vendida_neto,
                            (suma_comprada_neto - suma_vendida_neto) as diferencia
                            FROM 
                            (
                            select pol.date_planned, pol.name, substring(pol.name,2,18), sum(pol.product_qty) as suma_comprada_neto
                            ...
                            group by pol.date_planned, pol.name, substring(pol.name,2,18), pol.product_id
                            ) as sub_compra
                            INNER JOIN
                            (
                            ...
                            and sub_venta.fe_emision >= %s
                            and sub_venta.fe_emision <= %s
                            and sub_compra.date_planned >= %s 
                            and sub_compra.date_planned <= %s
                            ) 
                            as sub_final
                            where fe_corte_compra <= %s
                            GROUP BY fe_corte_venta, codigo_producto, nombre_producto, suma_vendida_neto
                            """,(data['date_from'],data['date_to'],data['date_from'],data['date_to'],data['date_to'])) 
